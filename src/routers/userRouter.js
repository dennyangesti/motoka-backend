...
router.post(`/users`, (req, res) => {
    ...
    const sql2 = `SELECT first_name, last_name, username, password, email, verified, phone_number, gender, address, avatar FROM users WHERE id = ?`
    const data = req.body


    if (data.first_name == '' || data.username == '' || data.password == '' || data.email == '') {
